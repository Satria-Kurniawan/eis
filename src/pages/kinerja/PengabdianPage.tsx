...
import { usePengabdian } from "@/hooks/kinerja/use-pengabdian";
import { useDebounce } from "@/hooks/use-debounce";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Skeleton } from "@/components/ui/skeleton";
import { RefreshCw, Handshake, AlertCircle, TrendingUp, Search, Filter } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { createColumns } from "./Pengabdian/columns";
import { PengabdianTable } from "./Pengabdian/pengabdian-table";
import { PengabdianDetailModal } from "./Pengabdian/pengabdian-detail-modal";
...
export default function PengabdianPage() {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = usePengabdian(debouncedSearch);

  const [selectedPengabdian, setSelectedPengabdian] = useState<Pengabdian | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [showFilters, setShowFilters] = useState(true);

  const handleViewDetail = (pengabdian: Pengabdian) => {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-2">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-indigo-500 font-bold text-xs uppercase tracking-[0.2em]">
            ...
            <span>Kinerja Pengabdian Masyarakat</span>
          </div>
          <h1 className="text-5xl font-black tracking-tighter text-foreground">
            Data Pengabdian Dosen
          </h1>
          <p className="text-muted-foreground font-medium max-w-xl text-lg">
            Monitor dan evaluasi rekam jejak kegiatan pengabdian kepada masyarakat yang dilakukan oleh dosen.
          </p>
        </div>
      ...
      {isError && (
        ...
        >
          <AlertTitle className="font-bold flex items-center gap-2">
            <AlertCircle className="size-4" /> Gagal Menarik Data Pengabdian
          </AlertTitle>
          <AlertDescription>
            ...
            >
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-card/40 backdrop-blur-md p-4 rounded-3xl border border-primary/5 shadow-sm mb-4">
                <div className="relative w-full sm:w-96 group">
                  ...
                  <Input
                    placeholder="Cari Nama Kegiatan atau Penulis..."
                    value={searchValue}
