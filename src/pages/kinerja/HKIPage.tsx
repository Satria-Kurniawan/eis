...
import { useHKI } from "@/hooks/kinerja/use-hki";
import { useDebounce } from "@/hooks/use-debounce";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Skeleton } from "@/components/ui/skeleton";
import { RefreshCw, ShieldCheck, AlertCircle, TrendingUp, Search, Filter } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { createColumns } from "./HKI/columns";
import { HKITable } from "./HKI/hki-table";
import { HKIDetailModal } from "./HKI/hki-detail-modal";
...
export default function HKIPage() {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useHKI(debouncedSearch);

  const [selectedHKI, setSelectedHKI] = useState<HKI | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [showFilters, setShowFilters] = useState(true);

  const handleViewDetail = (hki: HKI) => {
    setSelectedHKI(hki);
    setIsModalOpen(true);
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-2">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-indigo-500 font-bold text-xs uppercase tracking-[0.2em]">
            ...
            <span>Kinerja Hak Kekayaan Intelektual</span>
          </div>
          <h1 className="text-5xl font-black tracking-tighter text-foreground">
            Data HKI
          </h1>
          <p className="text-muted-foreground font-medium max-w-xl text-lg">
            Pantau dan lakukan monitoring rekam jejak sertifikasi Hak Kekayaan Intelektual (HKI) per periode akademik.
          </p>
        </div>
      ...
      {isError && (
        ...
        >
          <AlertTitle className="font-bold flex items-center gap-2">
            <AlertCircle className="size-4" /> Gagal Menarik Data HKI
          </AlertTitle>
          <AlertDescription>
            ...
            >
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-card/40 backdrop-blur-md p-4 rounded-3xl border border-primary/5 shadow-sm mb-4">
                <div className="relative w-full sm:w-96 group">
                  ...
                  <Input
                    placeholder="Cari Nama Karya atau Penulis..."
                    value={searchValue}
