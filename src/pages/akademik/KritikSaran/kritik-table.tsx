...
export function KritikTable<TData, TValue>({
  columns,
  data,
  pageCount,
  pageIndex,
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-1000">
      <div className="relative overflow-hidden rounded-3xl border border-primary/10 bg-card/60 backdrop-blur-md shadow-2xl transition-all">
        <Table>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="hover:bg-transparent border-primary/5">
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead key={header.id} className="h-14 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-6">
              ...
              <TableRow>
                ...
                >
                  <div className="flex flex-col items-center justify-center space-y-4 opacity-30">
                    ...
                    <span className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Tidak Ada Kritik atau Saran</span>
                  </div>
                </TableCell>
              </TableRow>
            )}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 bg-card/40 rounded-2xl border border-primary/5 shadow-sm">
        <div className="flex items-center gap-10">
          <div className="flex items-center gap-2">
            <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Limit</p>
            <Select
              value={`${pageSize}`}
              onValueChange={(value) => setPageSize(Number(value))}
            >
              <SelectTrigger className="h-9 w-[70px] bg-background/50 border-primary/10 rounded-xl font-bold text-xs">
                <SelectValue placeholder={pageSize} />
              </SelectTrigger>
              <SelectContent side="top" className="rounded-xl border-primary/10 shadow-xl">
                {[10, 20, 30, 40, 50].map((size) => (
                  <SelectItem key={size} value={`${size}`} className="rounded-lg font-bold text-xs">
                    {size}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em]">
            Halaman <span className="text-primary">{pageIndex}</span> / {pageCount}
          </div>
        </div>
