...
import { RefreshCw, Users, BookOpen, AlertCircle, ShieldCheck, Search, Filter } from "lucide-react";
...
export default function PegawaiPage() {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = usePegawaiHistory(debouncedSearch);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => refetch()}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all border border-primary/10 shadow-sm ${isFetching ? "bg-emerald-500/20 text-emerald-600 animate-pulse border border-emerald-500/20" : "bg-card hover:bg-muted text-muted-foreground"}`}
            >
              <RefreshCw
