...
import { useDosenHistory } from "@/hooks/kepegawaian/use-dosen";
import { useDebounce } from "@/hooks/use-debounce";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Skeleton } from "@/components/ui/skeleton";
import { RefreshCw, Users, BookOpen, AlertCircle, Search, Filter } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { useEffect, useState } from "react";
import { columns } from "./Dosen/columns";
import { DosenTable } from "./Dosen/dosen-table";
...
export default function DosenPage() {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useDosenHistory(debouncedSearch);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-2">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-indigo-500 font-bold text-xs uppercase tracking-[0.2em]">
            ...
            <span>Data Kepegawaian Akademik</span>
          </div>
          <h1 className="text-5xl font-black tracking-tighter text-foreground">
            History Data Dosen
          </h1>
          <p className="text-muted-foreground font-medium max-w-xl text-lg">
            Pantau dan lakukan monitoring rekam jejak tenaga pendidik per periode akademik menggunakan filter global.
          </p>
        </div>
      ...
      {isError && (
        ...
        >
          <AlertTitle className="font-bold flex items-center gap-2">
            <AlertCircle className="size-4" /> Gagal Menarik Data Dosen
          </AlertTitle>
          <AlertDescription>
